...
export default function DashboardNavbar({onToggleSidebar,isSidebarOpen}){
  ...
  return(
    <nav className="fixed top-0 left-0 right-0 z-50 bg-white border-b border-slate-100 h-[73px] flex items-center">
      <div className="flex items-center justify-between w-auto lg:w-64 px-2 sm:px-4 h-full shrink-0">
        <div className="flex items-center gap-1.5 sm:gap-3">
          <button onClick={onToggleSidebar} className="lg:hidden text-slate-500 hover:text-slate-900 transition-colors p-1.5 rounded-lg hover:bg-slate-100">
            {isSidebarOpen?<FaTimes size={18}/>:<FaBars size={18}/>}
          </button>
          <Link to={role.toLowerCase()==="admin"?"/admin":role.toLowerCase()==="manager"?"/manager":"/user"} className="flex items-center gap-1.5 sm:gap-2 min-w-0">
            <div className="p-1 rounded-lg border border-slate-200 shrink-0 bg-white">
              <img src={logo} alt="Shnoor" className="h-7 sm:h-8 w-auto object-contain"/>
            </div>
      ...
      <div className="flex-1 px-2 sm:px-6 lg:px-8 flex justify-end items-center h-full">
        <div className="flex items-center gap-2 sm:gap-6">
          <button onClick={toggleTheme} className="p-2 sm:p-2.5 bg-white border border-slate-100 rounded-xl shadow-sm text-slate-500 hover:text-slate-900 transition shrink-0">
            {theme==="light"?<FaSun size={16}/>:<FaMoon size={16}/>}
          </button>
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            <div className="w-8 h-8 sm:w-10 sm:h-10 bg-slate-50 rounded-xl flex items-center justify-center text-slate-600 font-bold border border-slate-100 shrink-0">
