  ...
  return(
    <AdminLayout>
      <div className="mb-6 lg:mb-8">
        <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 font-['Plus_Jakarta_Sans']">Skill Intelligence</h1>
        <p className="text-slate-500 text-sm mt-1 font-medium">Real-time demand analysis across all job postings</p>
      </div>
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 lg:gap-8">
        <div className="xl:col-span-8 bg-white border border-slate-100 rounded-2xl p-5 sm:p-6 lg:p-8 shadow-sm overflow-hidden">
          <h2 className="text-base sm:text-lg font-bold text-slate-900 mb-6 lg:mb-8 font-['Plus_Jakarta_Sans']">Top Demanded Skills</h2>
          <div className="space-y-6">
            {skills.map((s,i)=>(
              <div key={i}>
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 mb-2">
                  <span className="font-bold text-slate-900 break-all">{s.name}</span>
        ...
        <div className="xl:col-span-4 space-y-5 lg:space-y-6">
          <div className="bg-teal-500 p-5 sm:p-6 lg:p-8 rounded-2xl text-white shadow-xl shadow-teal-100">
            <h3 className="text-teal-100 text-xs font-bold uppercase tracking-widest mb-2">Primary Focus</h3>
            <p className="text-2xl sm:text-3xl font-bold break-all">{skills[0]?.name||"N/A"}</p>
          ...
          <div className="bg-white border border-slate-100 rounded-2xl p-5 sm:p-6 lg:p-8 shadow-sm">
            <h3 className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-4">Trending Tags</h3>
            <div className="flex flex-wrap gap-2">
              {skills.map((s,i)=>(
                <span key={i} className="px-3 py-1 bg-slate-50 text-slate-600 rounded-lg text-[10px] font-bold uppercase tracking-wider break-all">{s.name}</span>
              ))}
            </div>
          </div>
        </div>
