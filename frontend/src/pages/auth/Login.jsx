...
export default function Login(){
  ...
  return(
    <div className="min-h-screen bg-white flex relative">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-slate-50 items-center justify-center p-8 xl:p-16 border-r border-slate-100">
        <div className="max-w-md">
          ...
          <p className="text-slate-600 text-base xl:text-lg font-medium leading-relaxed mb-10">Sign in to manage your professional journey. Apply to elite roles, track your application status, and engage with top recruiters.</p>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-4 sm:p-6 lg:p-8 bg-white">
        <div className="w-full max-w-sm">
          ...
          <div className="mb-8 sm:mb-10">
            <h2 className="text-3xl sm:text-4xl font-black text-slate-900 mb-2 tracking-tight">Login</h2>
          ...
          <div className="space-y-4 sm:space-y-6">
            <div className="space-y-2">
              ...
              <input type="email" placeholder="name@email.com" value={form.email} onChange={(e)=>setForm({...form,email:e.target.value})} className="w-full p-4 sm:p-5 border border-slate-200 rounded-xl outline-none focus:border-teal-500 transition-all text-base font-medium" style={{backgroundColor:theme==="dark"?"#121212":"#ffffff",color:theme==="dark"?"#ffffff":"#0f172a"}}/>
            </div>
            <div className="space-y-2">
              <label className="text-xs font-bold text-slate-500 uppercase tracking-widest ml-1">Password</label>
              <div className="relative">
                <input type={showPassword?"text":"password"} placeholder="••••••••" value={form.password} onChange={(e)=>setForm({...form,password:e.target.value})} className="w-full p-4 sm:p-5 pr-14 border border-slate-200 rounded-xl outline-none focus:border-teal-500 transition-all text-base font-medium" style={{backgroundColor:theme==="dark"?"#121212":"#ffffff",color:theme==="dark"?"#ffffff":"#0f172a"}}/>
                <button type="button" onClick={()=>setShowPassword(!showPassword)} className="absolute right-4 sm:right-5 top-1/2 -translate-y-1/2 text-slate-400 hover:text-teal-600 transition">
                  {showPassword ? <FaEyeSlash size={18} /> : <FaEye size={18} />}
                </button>
              </div>
            </div>
          </div>
          <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mt-6 mb-10">
          ...
          <button onClick={handleLogin} disabled={loading} className="w-full py-4 sm:py-5 bg-primary text-white rounded-xl font-bold text-sm uppercase tracking-widest hover:bg-secondary transition-all shadow-lg active:scale-95 disabled:opacity-50">
