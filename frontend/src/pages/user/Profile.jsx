          ...
          <div className="w-64 border-r border-slate-200 bg-white p-4 space-y-1">
            ...
            {tabs.map(t => (
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-2.5 text-sm font-medium rounded transition-all ${activeTab === t.name ? "bg-teal-50 text-teal-700" : "text-slate-600 hover:bg-slate-50"}`}
              >
          ...
          <div className="flex-1 p-10 max-w-5xl">
            {activeTab === "Personal Details" && (
              <div className="space-y-10">
                <div className="flex items-center gap-8 pb-10 border-b border-slate-200">
                  <div className="w-24 h-24 rounded border border-slate-200 p-1 relative group">
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between items-start">
                      <div className="flex-1 mr-4">
                        <h2 className="text-3xl font-bold text-slate-900">{profile.first_name || "New User"}</h2>
                        {isEditing ? (
                ...
                <div className="grid grid-cols-2 gap-x-12 gap-y-8">
                  <div className="col-span-2"><h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest border-b border-slate-100 pb-2 mb-4">Basic Information</h3></div>
                  {isEditing ? (
                    <>
                      ...
                      <div className="col-span-2"><label className={labelC}>Professional Summary</label><textarea className={`${inputC} h-32 py-4`} value={editForm.summary || ""} onChange={e => setEditForm({ ...editForm, summary: e.target.value })} placeholder="Briefly describe your professional journey and key expertise..." /></div>
                    </>
                  ) : (
                    <>
                      ...
                      <div className="col-span-2">{infoRow("Professional Summary", profile.summary)}</div>
                    </>
                  )}

                  <div className="col-span-2 mt-8"><h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest border-b border-slate-100 pb-2 mb-4">Contact & Location</h3></div>
                  {isEditing ? (
                    <>
                      <div className="col-span-2"><label className={labelC}>Address</label><input className={inputC} value={editForm.personal_info?.address || ""} onChange={e => setEditForm({ ...editForm, personal_info: { ...editForm.personal_info, address: e.target.value } })} /></div>
                    ...
                    <>
                      <div className="col-span-2">{infoRow("Address", profile.personal_info?.address)}</div>
                ...
                <div className="space-y-12">
                  ...
                  {((isEditing ? editForm.experiences : profile.experiences) || []).length > 0 ? ((isEditing ? editForm.experiences : profile.experiences || [])).map((exp, i) => (
                    <div key={i} className="relative border-l-2 border-slate-100 pl-8 ml-2 group">
                      {isEditing ? (
                        <div className="relative grid grid-cols-2 gap-x-6 gap-y-4 bg-slate-50/50 p-6 rounded-lg border border-slate-100">
                          ...
                          <div className="col-span-2"><label className={labelC}>Role Description</label><textarea className={`${inputC} h-24`} value={exp.desc || ""} onChange={e => { const n = [...editForm.experiences]; n[i].desc = e.target.value; setEditForm({ ...editForm, experiences: n }) }} /></div>
                        </div>
                      ) : (
                        <div>
                  ...
                  {((isEditing ? editForm.education : profile.education) || []).some(e => e.institution || isEditing) ? ((isEditing ? editForm.education : profile.education || [])).map((edu, i) => (
                    edu.institution || isEditing ? (
                      <div key={i} className="border-l-2 border-slate-100 pl-8 ml-2">
                        ...
                        {isEditing ? (
                          <div className="grid grid-cols-2 gap-x-6 gap-y-4 bg-slate-50/50 p-6 rounded-lg border border-slate-100">
                            <div className="col-span-2"><label className={labelC}>Institution / University</label><input className={inputC} value={edu.institution || ""} onChange={e => { const n = [...editForm.education]; n[i].institution = e.target.value; setEditForm({ ...editForm, education: n }) }} /></div>
                    ...
                    <div key={i} className="relative p-6 border border-slate-200 rounded group hover:border-teal-200 transition-all">
                      {isEditing ? (
                        <div className="relative grid grid-cols-2 gap-4">
                          ...
                          <div className="col-span-2"><label className={labelC}>Description</label><textarea className={`${inputC} h-20`} value={p.desc || ""} onChange={e => { const n = [...editForm.projects]; n[i].desc = e.target.value; setEditForm({ ...editForm, projects: n }) }} /></div>
                        </div>
                      ) : (
                        <div>
                          <div className="flex justify-between items-start">
              ...
              <div className="max-w-md space-y-8">
                ...
                <div className="pt-10 border-t border-slate-100 mt-10">
                  ...
                  <div className="p-6 border border-red-100 rounded-2xl bg-red-50/30 flex items-center justify-between">
                    ...
                    <button onClick={async () => { if (window.confirm("WARNING: Permanent deletion. Continue?")) { try { await api.delete("/api/profile/delete/"); sessionStorage.clear(); navigate("/"); window.location.reload(); } catch { alert("Failed"); } } }} className="px-6 py-2.5 bg-red-500 text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-red-600 transition-all shadow-lg shadow-red-100">Delete Permanently</button>
                  </div>
                </div>
              </div>
            )}
