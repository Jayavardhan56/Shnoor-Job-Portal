  ...
  return(
    <ManagerLayout>
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-10">
        ...
        <button onClick={()=>window.location.href='/manager/messages'} className="px-6 py-4 bg-primary text-white rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-secondary flex items-center gap-3 w-fit">
          <FaCommentAlt size={14}/>
          Messages
          {notifCount>0&&(
            <span className="bg-red-50 text-white text-xs rounded-full px-1.5 py-0.5 font-bold">{notifCount}</span>
          )}
        </button>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
        <div className="bg-teal-50 p-5 sm:p-6 lg:p-10 rounded-2xl border border-teal-100 shadow-sm relative overflow-hidden">
        ...
        <div className="bg-white p-5 sm:p-6 lg:p-10 rounded-2xl border border-slate-200 shadow-sm">
        ...
        <div className="bg-teal-50 p-5 sm:p-6 lg:p-10 rounded-2xl border border-teal-100 shadow-sm flex flex-col justify-center">
      ...
      <div className="bg-white p-5 sm:p-6 lg:p-10 rounded-2xl border border-slate-100 shadow-sm">
        <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-10">
          ...
          <button onClick={()=>window.location.href='/manager/jobs'} className="px-8 py-3 bg-slate-50 text-slate-600 rounded-xl text-sm font-bold tracking-widest uppercase hover:bg-slate-100 transition shadow-sm border border-slate-200 w-fit">Manage Pipeline</button>
        </div>
        <div className="grid md:grid-cols-2 gap-8">
          {jobs.length===0?(
            <div className="col-span-1 md:col-span-2 py-20 text-center bg-slate-50 rounded-2xl border border-dashed border-slate-300">
